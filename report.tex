...
maximized with respect to $p^{t+1}( c=f \mid x^d_{ij}, s_j)$:
\[
...
\]
The parameters $\mu_2^{t+1}, \sigma_1^{t+1}, \sigma_2^{t+1}$ are
calculated analogously. Estimation alternates between E-step and
M-step until the stopping condition
$|1-\frac{\mu_1^{t+1}}{\mu_1^{t}}| < \epsilon \wedge
|1-\frac{\mu_2^{t+1}}{\mu_2^{t}}| < \epsilon$ holds, where
...
s_{i-1}=\text{w}) = 2-\sqrt{\frac{4}{E[W]}}.
\]
